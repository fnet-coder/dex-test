...
local function main()
	...
	SettingsWindow.Init = function()
		...
		local themeChooser = AddDropdown("Theme", {"Classic", "BlueScreen", "Honey", "Agent"}, Settings.ThemeName or "Classic", false, 100)
		themeChooser.OnSelect:Connect(function()
			Settings.ThemeName = themeChooser.Selected
			Main.ApplyThemePreset(Settings.ThemeName)
...
Main = (function()
	...
	Main.ApplyThemePreset = function(name)
		local presets = {
			Classic = {
				Main1 = Color3.fromRGB(52,52,52), Main2 = Color3.fromRGB(45,45,45),
				Outline1 = Color3.fromRGB(33,33,33), Outline2 = Color3.fromRGB(55,55,55),
				Outline3 = Color3.fromRGB(30,30,30), TextBox = Color3.fromRGB(38,38,38),
				Menu = Color3.fromRGB(32,32,32), ListSelection = Color3.fromRGB(11,90,175),
				Button = Color3.fromRGB(60,60,60), ButtonHover = Color3.fromRGB(68,68,68),
				ButtonPress = Color3.fromRGB(40,40,40), Highlight = Color3.fromRGB(75,75,75),
				Text = Color3.fromRGB(255,255,255), PlaceholderText = Color3.fromRGB(100,100,100),
				Important = Color3.fromRGB(255,0,0),
				Syntax = {Background = Color3.fromRGB(36,36,36), Text = Color3.fromRGB(204,204,204), SelectionBack = Color3.fromRGB(11,90,175)}
			},
			BlueScreen = {
				Main1 = Color3.fromRGB(8,42,92), Main2 = Color3.fromRGB(5,31,72),
				Outline1 = Color3.fromRGB(26,101,178), Outline2 = Color3.fromRGB(43,132,214),
				Outline3 = Color3.fromRGB(3,22,52), TextBox = Color3.fromRGB(7,39,86),
				Menu = Color3.fromRGB(4,26,61), ListSelection = Color3.fromRGB(35,139,230),
				Button = Color3.fromRGB(16,77,145), ButtonHover = Color3.fromRGB(25,103,184),
				ButtonPress = Color3.fromRGB(8,50,105), Highlight = Color3.fromRGB(44,157,239),
				Text = Color3.fromRGB(245,250,255), PlaceholderText = Color3.fromRGB(151,191,226),
				Important = Color3.fromRGB(255,222,89),
				Syntax = {Background = Color3.fromRGB(4,25,58), Text = Color3.fromRGB(220,239,255), SelectionBack = Color3.fromRGB(30,116,202)}
			},
			Honey = {
				Main1 = Color3.fromRGB(66,47,18), Main2 = Color3.fromRGB(47,34,15),
				Outline1 = Color3.fromRGB(107,72,20), Outline2 = Color3.fromRGB(138,93,25),
				Outline3 = Color3.fromRGB(35,25,11), TextBox = Color3.fromRGB(55,39,15),
				Menu = Color3.fromRGB(40,28,11), ListSelection = Color3.fromRGB(207,139,27),
				Button = Color3.fromRGB(115,76,18), ButtonHover = Color3.fromRGB(155,103,23),
				ButtonPress = Color3.fromRGB(78,50,12), Highlight = Color3.fromRGB(239,177,43),
				Text = Color3.fromRGB(255,246,214), PlaceholderText = Color3.fromRGB(190,158,96),
				Important = Color3.fromRGB(255,101,61),
		...
		local preset = presets[name] or presets.Classic
		local theme = Settings.Theme
		for key,value in pairs(preset) do
			if key ~= "Syntax" then theme[key] = value end
		end
		for key,value in pairs(preset.Syntax) do theme.Syntax[key] = value end
		Settings.ThemeName = (presets[name] and name) or "Classic"
	end

	Main.GetProductName = function()
		return Settings.ThemeName == "Agent" and "Dex Agent" or "Dex+++"
	end
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or Main.GetProductName()
	...
	Main.InitReplay = function()
		...
		local function stopPlayback()
			...
			if replay.Playing then
				...
				if replay.SavedCameraSubject then camera.CameraSubject = replay.SavedCameraSubject end
			end
			replay.Playing = false
			setProgress(0)
			setStatus(#replay.Frames > 0 and ("Ready - "..tostring(#replay.Frames).." frames") or "No replay recorded")
		end

		local function stopRecording(disableAuto)
			if replay.CaptureConnection then replay.CaptureConnection:Disconnect() replay.CaptureConnection = nil end
			replay.Recording = false
			if disableAuto then replay.AutoRecord = false end
			setStatus(#replay.Frames > 0 and ("Recorded "..string.format("%.1fs",replay.Elapsed)) or "No replay recorded")
		end

		local function startRecording(reset)
			replay.AutoRecord = true
		...
		local function playReplay()
			if #replay.Frames == 0 then setStatus("Record something first") return end
			stopRecording(false)
			stopPlayback()
			local camera = workspace.CurrentCamera
			clearGhosts()
			...
			camera.CameraType = Enum.CameraType.Scriptable
			replay.Playing = true
			replay.PlaybackStart = os.clock()
			local playbackOrigin = replay.Frames[1].T
			setStatus("Playing replay")
			replay.PlaybackConnection = runService.RenderStepped:Connect(function()
				if not replay.Playing then return end
				local elapsed = playbackOrigin + os.clock() - replay.PlaybackStart
				local first = replay.Frames[1]
				local last = replay.Frames[#replay.Frames]
		...
		local function saveReplay()
			if not env.writefile or #replay.Frames == 0 then setStatus("Filesystem unavailable or replay empty") return end
			local ok,data = pcall(httpService.JSONEncode,httpService,replay.Frames)
			if ok then
				env.writefile("DexReplay.json",data)
				setStatus("Replay saved to DexReplay.json")
			end
		end

		local function loadReplay()
			if not env.readfile then setStatus("Filesystem unavailable") return end
			local ok,data = pcall(env.readfile,"DexReplay.json")
			if not ok or not data then setStatus("DexReplay.json not found") return end
			local decoded,frames = pcall(httpService.JSONDecode,httpService,data)
			if not decoded or type(frames) ~= "table" or #frames == 0 then setStatus("Invalid replay file") return end
			replay.Frames = frames
			replay.Elapsed = frames[#frames].T or 0
			setStatus("Loaded "..tostring(#frames).." frames")
		end

		replay.StartRecording = startRecording
		replay.StopRecording = function() stopRecording(true) end
		replay.PlayReplay = playReplay
		replay.StopPlayback = stopPlayback
		replay.SaveReplay = saveReplay
		replay.LoadReplay = loadReplay

		local window = Lib.Window.new()
		window:SetTitle("Replay Studio")
		window:Resize(360,155)
		...
		replay.Status = status.Gui
		local progressBack = Instance.new("Frame")
		progressBack.BackgroundColor3 = Settings.Theme.Outline3
		progressBack.BorderSizePixel = 0
		progressBack.Position = UDim2.new(0,8,0,38)
		progressBack.Size = UDim2.new(1,-16,0,6)
		window:Add(progressBack)
		local progress = Instance.new("Frame")
		progress.BackgroundColor3 = Settings.Theme.Highlight
		progress.BorderSizePixel = 0
		progress.Size = UDim2.new(0,0,1,0)
		progress.Parent = progressBack
		replay.Progress = progress
		local buttons = {{"Record",function() startRecording(true) end},{"Stop",function() stopRecording(true) stopPlayback() end},{"Replay",playReplay},{"Clear",function() stopRecording(true) stopPlayback() table.clear(replay.Frames) replay.Elapsed = 0 setProgress(0) setStatus("Replay cleared") end},{"Save",saveReplay},{"Load",loadReplay}}
		for i,data in ipairs(buttons) do
			local button = Lib.Button.new()
			button.Text = data[1]
			button.Size = UDim2.new(0,105,0,23)
			button.Position = UDim2.new(0,8 + ((i-1)%3)*115,0,52 + math.floor((i-1)/3)*30)
			button.OnClick:Connect(data[2])
			window:Add(button)
		end
		Replay = replay
		Main.Replay = replay
	...
	Main.CreateMainGui = function()
		...
		gui.OpenButton.Text = Main.GetProductName()
	...
	-- Public utility surface for plugins, console commands, and future UI panels.
	Main.Features = Main.Features or {}
	Main.FeatureOrder = {
		"GetSelection", "GetSelectionPaths", "CopySelectionPaths", "RefreshExplorer",
		"SelectChildren", "SelectParents", "ExpandSelection", "CollapseSelection",
		"ViewSelectedScript", "ViewSelectedModel", "TeleportToSelection", "DuplicateSelection",
		"DeleteSelection", "RenameSelection", "ClearConsole", "SaveSelection",
		"ShowExplorer", "ShowProperties", "ShowConsole", "ShowModelViewer",
		"SelectDescendants", "SelectSiblings", "ClearSelection", "FocusSelection",
		"GetSelectionSummary", "CopySelectionSummary", "GetClassCounts", "CopyClassCounts",
		"FindInstances", "SaveSettings", "SetTheme",
		"GetSelectionCount", "GetPrimarySelection", "GetPrimaryPath", "CopyPrimaryPath",
		"GetSelectedNames", "CopySelectedNames", "GetSelectionClasses", "CopySelectionClasses",
		"SelectByClass", "SelectByName", "SelectRoots", "SelectTopLevel",
		"SelectAncestors", "ExpandParents", "CollapseChildren", "ViewParent",
		"CopyParentPath", "GetDescendantCount", "GetChildCount", "HasChildren",
		"SortSelectionByName", "ReverseSelection", "RemoveSelectionDuplicates", "RefreshSelection",
		"SelectWorkspace", "SelectLocalPlayer", "SelectCharacter", "SelectPlayerGui",
		"FindServices", "FindScripts", "FindModels", "FindRemotes", "FindGuiObjects",
		"FindParts", "FindSounds", "ToggleExplorer", "ToggleProperties", "ToggleConsole",
		"ToggleModelViewer", "HideAllWindows", "ShowAllWindows", "ResetSettings",
		"SetClassIcon", "SetWindowTransparency", "SetTitleCentered", "GetThemeName",
		"GetVersion", "GetMissingEnvironment", "GetSelectedFullNames", "CopySelectedFullNames",
		"InspectSelection", "CopyInspection", "GetAncestrySnapshot", "CopyAncestrySnapshot",
	...
	local function selectedNodes()
		return Explorer and Explorer.Selection and Explorer.Selection.List or {}
	end

	local function selectedObjects()
		local result = {}
		for _,node in ipairs(selectedNodes()) do
			if node and node.Obj then result[#result+1] = node.Obj end
		end
		return result
	end

	Main.Features.GetSelection = selectedObjects
	Main.Features.GetSelectionPaths = function()
		local result = {}
		for _,obj in ipairs(selectedObjects()) do
			result[#result+1] = Explorer.GetInstancePath(obj)
		end
		return result
	end
	Main.Features.CopySelectionPaths = function()
		if not env.setclipboard then return false end
		env.setclipboard(table.concat(Main.Features.GetSelectionPaths(), "\n"))
		return true
	end
	Main.Features.RefreshExplorer = function()
		if not Explorer then return false end
		Explorer.ForceUpdate()
		return true
	end
	Main.Features.SelectChildren = function()
		local result = {}
		for _,node in ipairs(selectedNodes()) do
			for _,child in ipairs(node) do result[#result+1] = child end
		end
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.SelectDescendants = function()
		local result,seen = {},{}
		local function addChildren(node)
		...
		for _,node in ipairs(selectedNodes()) do addChildren(node) end
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.SelectSiblings = function()
		local result,seen = {},{}
		for _,node in ipairs(selectedNodes()) do
			local parent = node.Parent
			if parent then
				for _,sibling in ipairs(parent) do
					if not seen[sibling] then
						seen[sibling] = true
	...
	Main.Features.ClearSelection = function()
		Explorer.Selection:Clear()
		return true
	end
	Main.Features.FocusSelection = function()
		local node = selectedNodes()[1]
		if not node then return false end
		Explorer.ViewNode(node)
		return true
	end
	Main.Features.SelectParents = function()
		local result,seen = {},{}
		for _,node in ipairs(selectedNodes()) do
			local parent = node.Parent
	...
	Main.Features.ExpandSelection = function()
		for _,node in ipairs(selectedNodes()) do Explorer.MakeNodeVisible(node,true) end
		return Main.Features.RefreshExplorer()
	end
	Main.Features.CollapseSelection = function()
		for _,node in ipairs(selectedNodes()) do Explorer.Expanded[node] = nil end
		return Main.Features.RefreshExplorer()
	end
	Main.Features.ViewSelectedScript = function()
		local obj = selectedObjects()[1]
		if obj and obj:IsA("LuaSourceContainer") then ScriptViewer.ViewScript(obj) return true end
		return false
	end
	Main.Features.ViewSelectedModel = function()
		local obj = selectedObjects()[1]
		if obj and (obj:IsA("BasePart") or obj:IsA("Model")) then ModelViewer.ViewModel(obj) return true end
		return false
	end
	Main.Features.TeleportToSelection = function()
		local obj = selectedObjects()[1]
		local root = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
		...
		root.CFrame = CFrame.new(part.Position + (Settings.Explorer.TeleportToOffset or Vector3.zero))
		return true
	end
	Main.Features.DuplicateSelection = function()
		local count = 0
		for _,obj in ipairs(selectedObjects()) do
			local ok,copy = pcall(obj.Clone,obj)
			if ok and copy and obj.Parent then copy.Parent = obj.Parent count = count + 1 end
		end
		Main.Features.RefreshExplorer()
		return count
	end
	Main.Features.DeleteSelection = function()
		local count = 0
		for _,obj in ipairs(selectedObjects()) do
			if obj ~= game and obj.Parent then pcall(obj.Destroy,obj) count = count + 1 end
		end
		Explorer.Selection:Clear()
		return count
	end
	Main.Features.RenameSelection = function(name)
		local obj = selectedObjects()[1]
		if not obj or type(name) ~= "string" or name == "" then return false end
		local ok = pcall(function() obj.Name = name end)
		Main.Features.RefreshExplorer()
		return ok
	end
	Main.Features.GetSelectionSummary = function()
		local lines = {}
		for _,obj in ipairs(selectedObjects()) do
			lines[#lines+1] = ("[%s] %s\n%s"):format(obj.ClassName,obj.Name,Explorer.GetInstancePath(obj))
		end
		return table.concat(lines,"\n")
	end
	Main.Features.CopySelectionSummary = function()
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.GetSelectionSummary())
		return true
	end
	Main.Features.GetClassCounts = function()
		local counts = {}
		for _,obj in ipairs(selectedObjects()) do
	...
	Main.Features.CopyClassCounts = function()
		if not env.setclipboard then return false end
		local counts,lines = Main.Features.GetClassCounts(),{}
		for className,count in pairs(counts) do lines[#lines+1] = className.." = "..count end
		table.sort(lines)
		env.setclipboard(table.concat(lines,"\n"))
		return true
	end
	Main.Features.FindInstances = function(query,className)
		if type(query) ~= "string" or query == "" then return 0 end
		local result,lowerQuery = {},string.lower(query)
		for _,node in pairs(nodes) do
			local obj = node.Obj
			if obj and (not className or obj:IsA(className)) and string.find(string.lower(obj.Name),lowerQuery,1,true) then
				result[#result+1] = node
			end
		end
		Explorer.Selection:SetTable(result)
		if result[1] then Explorer.ViewNode(result[1]) end
		return #result
	end
	Main.Features.ClearConsole = function()
		if not Console or not Console.Window then return false end
		local output = Console.Window.GuiElems.Content:FindFirstChild("Console",true)
		local frame = output and output:FindFirstChild("Output")
		if not frame then return false end
		for _,child in ipairs(frame:GetChildren()) do if child:IsA("TextBox") then child:Destroy() end end
		return true
	end
	Main.Features.SaveSelection = function()
		local obj = selectedObjects()[1]
		if not obj or not env.saveinstance then return false end
		Lib.SaveAsPrompt("Place_"..game.PlaceId.."_"..obj.ClassName.."_"..obj.Name.."_"..os.time(), function(filename)
			env.saveinstance(obj,filename,{Decompile=true,RemovePlayerCharacters=false})
		end)
		return true
	end
	Main.Features.ShowExplorer = function() Explorer.Window:Show() return true end
	Main.Features.ShowProperties = function() Properties.Window:Show() return true end
	Main.Features.ShowConsole = function() Console.Window:Show() return true end
	Main.Features.ShowModelViewer = function() ModelViewer.Window:Show() return true end
	Main.Features.SaveSettings = function()
		Main.SaveCurrentSettings()
		return true
	end
	Main.Features.SetTheme = function(name)
		if type(name) ~= "string" then return false end
		Main.ApplyThemePreset(name)
		Main.SaveCurrentSettings()
		if SettingsWindow and SettingsWindow.ReloadPrompt then SettingsWindow.ReloadPrompt() end
		return Settings.ThemeName == name
	end

	local function selectMatching(predicate)
		local result = {}
		for _,node in pairs(nodes) do
			local ok,matched = pcall(predicate,node.Obj)
			if ok and matched then result[#result+1] = node end
		end
		Explorer.Selection:SetTable(result)
		if result[1] then Explorer.ViewNode(result[1]) end
		return #result
	...
	Main.Features.GetSelectionCount = function() return #selectedNodes() end
	Main.Features.GetPrimarySelection = function() return selectedObjects()[1] end
	Main.Features.GetPrimaryPath = function()
		local obj = selectedObjects()[1]
		return obj and Explorer.GetInstancePath(obj) or nil
	end
	Main.Features.CopyPrimaryPath = function()
		local path = Main.Features.GetPrimaryPath()
		if not path or not env.setclipboard then return false end
		env.setclipboard(path)
		return true
	end
	Main.Features.GetSelectedNames = function()
		local result = {}
		for _,obj in ipairs(selectedObjects()) do result[#result+1] = obj.Name end
		return result
	end
	Main.Features.CopySelectedNames = function()
		if not env.setclipboard then return false end
		env.setclipboard(table.concat(Main.Features.GetSelectedNames(),"\n"))
		return true
	end
	Main.Features.GetSelectionClasses = function()
		local result,seen = {},{}
		for _,obj in ipairs(selectedObjects()) do
			if not seen[obj.ClassName] then seen[obj.ClassName] = true result[#result+1] = obj.ClassName end
		end
		table.sort(result)
		return result
	end
	Main.Features.CopySelectionClasses = function()
		if not env.setclipboard then return false end
		env.setclipboard(table.concat(Main.Features.GetSelectionClasses(),"\n"))
		return true
	end
	Main.Features.SelectByClass = function(className)
		if type(className) ~= "string" or className == "" then return 0 end
		return selectMatching(function(obj) return obj:IsA(className) end)
	end
	Main.Features.SelectByName = function(name,exact)
		if type(name) ~= "string" or name == "" then return 0 end
		local query = string.lower(name)
		return selectMatching(function(obj)
			local value = string.lower(obj.Name)
			return exact and value == query or string.find(value,query,1,true) ~= nil
		end)
	end
	Main.Features.SelectRoots = function()
		local result,seen = {},{}
		for _,node in ipairs(selectedNodes()) do
			local current = node
			while current.Parent and selectedNodes() and Explorer.Selection.Map[current.Parent] do current = current.Parent end
			if not seen[current] then seen[current] = true result[#result+1] = current end
		end
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.SelectTopLevel = function()
		return selectMatching(function(obj) return obj.Parent == game end)
	end
	Main.Features.SelectAncestors = function()
		local result,seen = {},{}
		for _,node in ipairs(selectedNodes()) do
	...
	Main.Features.ExpandParents = function()
		for _,node in ipairs(selectedNodes()) do Explorer.MakeNodeVisible(node) end
		return Main.Features.RefreshExplorer()
	end
	Main.Features.CollapseChildren = function()
		for _,node in ipairs(selectedNodes()) do Explorer.Expanded[node] = nil end
		return Main.Features.RefreshExplorer()
	end
	Main.Features.ViewParent = function()
		local node = selectedNodes()[1]
		if not node or not node.Parent then return false end
		Explorer.Selection:Set(node.Parent)
		Explorer.ViewNode(node.Parent)
		return true
	end
	Main.Features.CopyParentPath = function()
		local node = selectedNodes()[1]
		local parent = node and node.Parent
		if not parent or not env.setclipboard then return false end
		env.setclipboard(Explorer.GetInstancePath(parent.Obj))
		return true
	end
	Main.Features.GetDescendantCount = function()
		local total = 0
		for _,obj in ipairs(selectedObjects()) do total = total + #obj:GetDescendants() end
		return total
	end
	Main.Features.GetChildCount = function()
		local total = 0
		for _,obj in ipairs(selectedObjects()) do total = total + #obj:GetChildren() end
		return total
	end
	Main.Features.HasChildren = function()
		for _,obj in ipairs(selectedObjects()) do if #obj:GetChildren() > 0 then return true end end
		return false
	end
	Main.Features.SortSelectionByName = function()
		local result = selectedNodes()
		table.sort(result,function(a,b) return a.Obj.Name:lower() < b.Obj.Name:lower() end)
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.ReverseSelection = function()
		local result = {}
		for i = #selectedNodes(),1,-1 do result[#result+1] = selectedNodes()[i] end
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.RemoveSelectionDuplicates = function()
		local result,seen = {},{}
		for _,node in ipairs(selectedNodes()) do if not seen[node] then seen[node] = true result[#result+1] = node end end
		Explorer.Selection:SetTable(result)
		return #result
	end
	Main.Features.RefreshSelection = function()
		if Explorer.UpdateSelectionVisuals then Explorer.UpdateSelectionVisuals() end
		Explorer.Refresh()
		return true
	end
	Main.Features.SelectWorkspace = function()
		return selectMatching(function(obj) return obj == workspace end)
	end
	Main.Features.SelectLocalPlayer = function()
		return selectMatching(function(obj) return obj == plr end)
	end
	Main.Features.SelectCharacter = function()
		return selectMatching(function(obj) return plr.Character and obj == plr.Character end)
	end
	Main.Features.SelectPlayerGui = function()
		return selectMatching(function(obj) return plr:FindFirstChildOfClass("PlayerGui") == obj end)
	end
	Main.Features.FindServices = function(query)
		local lowerQuery = query and string.lower(query)
		return selectMatching(function(obj)
			local class = API.Classes[obj.ClassName]
			return class and class.Tags and class.Tags.Service and (not lowerQuery or string.find(string.lower(obj.Name),lowerQuery,1,true))
		end)
	end
	Main.Features.FindScripts = function(query) return Main.Features.FindInstances(query,"LuaSourceContainer") end
	Main.Features.FindModels = function(query) return Main.Features.FindInstances(query,"Model") end
	Main.Features.FindRemotes = function(query) return selectMatching(function(obj) return (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") or obj:IsA("UnreliableRemoteEvent")) and (not query or string.find(string.lower(obj.Name),string.lower(query),1,true)) end) end
	Main.Features.FindGuiObjects = function(query) return selectMatching(function(obj) return obj:IsA("GuiObject") and (not query or string.find(string.lower(obj.Name),string.lower(query),1,true)) end) end
	Main.Features.FindParts = function(query) return selectMatching(function(obj) return obj:IsA("BasePart") and (not query or string.find(string.lower(obj.Name),string.lower(query),1,true)) end) end
	Main.Features.FindSounds = function(query) return selectMatching(function(obj) return obj:IsA("Sound") and (not query or string.find(string.lower(obj.Name),string.lower(query),1,true)) end) end
	Main.Features.ToggleExplorer = function() if Explorer.Window:IsVisible() then Explorer.Window:Hide() else Explorer.Window:Show() end return true end
	Main.Features.ToggleProperties = function() if Properties.Window:IsVisible() then Properties.Window:Hide() else Properties.Window:Show() end return true end
	Main.Features.ToggleConsole = function() if Console.Window:IsVisible() then Console.Window:Hide() else Console.Window:Show() end return true end
	Main.Features.ToggleModelViewer = function() if ModelViewer.Window:IsVisible() then ModelViewer.Window:Hide() else ModelViewer.Window:Show() end return true end
	Main.Features.HideAllWindows = function()
		for _,app in ipairs({Explorer,Properties,Console,ModelViewer,ScriptViewer,SaveInstance,SettingsWindow}) do if app and app.Window then app.Window:Hide() end end
		return true
	end
	Main.Features.ShowAllWindows = function()
		for _,app in ipairs({Explorer,Properties,Console,ModelViewer,ScriptViewer,SaveInstance,SettingsWindow}) do if app and app.Window then app.Window:Show() end end
		return true
	end
	Main.Features.ResetSettings = function()
		Main.ResetSettings()
		Main.SaveCurrentSettings()
		return true
	end
	Main.Features.SetClassIcon = function(name)
		local ok = type(name) == "string" and pcall(Lib.IconMap.getIconDataFromName,name)
		if not ok then return false end
		Settings.ClassIcon = name
		Main.SaveCurrentSettings()
		return true
	end
	Main.Features.SetWindowTransparency = function(value)
		value = tonumber(value)
		if not value then return false end
		Settings.Window.Transparency = math.clamp(value,0,1)
		Main.SaveCurrentSettings()
		return true
	end
	Main.Features.SetTitleCentered = function(value)
		Settings.Window.TitleOnMiddle = value == true
		Main.SaveCurrentSettings()
		return Settings.Window.TitleOnMiddle
	end
	Main.Features.GetThemeName = function() return Settings.ThemeName end
	Main.Features.GetVersion = function() return Main.Version end
	Main.Features.GetMissingEnvironment = function() return Main.MissingEnv end
	Main.Features.GetSelectedFullNames = function()
		local result = {}
		for _,obj in ipairs(selectedObjects()) do result[#result+1] = obj:GetFullName() end
		return result
	end
	Main.Features.CopySelectedFullNames = function()
		if not env.setclipboard then return false end
		env.setclipboard(table.concat(Main.Features.GetSelectedFullNames(),"\n"))
		return true
	end
	Main.Features.InspectSelection = function()
		local lines = {}
		for _,obj in ipairs(selectedObjects()) do
			local parent = obj.Parent and obj.Parent:GetFullName() or "nil"
			lines[#lines+1] = ("Name: %s\nClass: %s\nPath: %s\nParent: %s\nChildren: %d\nDescendants: %d"):format(
				obj.Name,obj.ClassName,Explorer.GetInstancePath(obj),parent,#obj:GetChildren(),#obj:GetDescendants())
		end
		return table.concat(lines,"\n\n")
	end
	Main.Features.CopyInspection = function()
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.InspectSelection())
		return true
	end
	Main.Features.GetAncestrySnapshot = function()
		local result = {}
		for _,obj in ipairs(selectedObjects()) do
			local chain = {}
			local current = obj
			while current do
				chain[#chain+1] = current:GetFullName()
				current = current.Parent
			end
			result[#result+1] = table.concat(chain,"\n")
		end
		return table.concat(result,"\n\n")
	end
	Main.Features.CopyAncestrySnapshot = function()
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.GetAncestrySnapshot())
		return true
	end
	Main.Features.GetAttributeSnapshot = function()
		local lines = {}
		for _,obj in ipairs(selectedObjects()) do
			local attributes = obj:GetAttributes()
			lines[#lines+1] = "["..obj:GetFullName().."]"
			for name,value in pairs(attributes) do lines[#lines+1] = name.." = "..tostring(value) end
		end
		return table.concat(lines,"\n")
	end
	Main.Features.CopyAttributeSnapshot = function()
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.GetAttributeSnapshot())
		return true
	end
	Main.Features.GetPropertySnapshot = function()
		local lines = {}
		for _,obj in ipairs(selectedObjects()) do
			local class = API.Classes[obj.ClassName]
			lines[#lines+1] = "["..obj:GetFullName().."]"
			while class do
				for _,prop in ipairs(class.Properties or {}) do
					...
					if ok then lines[#lines+1] = prop.Name.." = "..tostring(value) end
				end
	...
	Main.Features.CopyPropertySnapshot = function()
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.GetPropertySnapshot())
		return true
	end
	Main.Features.GetTreeSnapshot = function(depth)
		local maxDepth = math.clamp(tonumber(depth) or 2,0,10)
		local lines = {}
		local function append(obj,currentDepth)
			lines[#lines+1] = string.rep("  ",currentDepth)..obj.Name.." ["..obj.ClassName.."]"
			if currentDepth >= maxDepth then return end
			for _,child in ipairs(obj:GetChildren()) do append(child,currentDepth+1) end
		end
		for _,obj in ipairs(selectedObjects()) do append(obj,0) end
		return table.concat(lines,"\n")
	end
	Main.Features.CopyTreeSnapshot = function(depth)
		if not env.setclipboard then return false end
		env.setclipboard(Main.Features.GetTreeSnapshot(depth))
		return true
	end
	Main.Features.GetClassMetadata = function()
		local obj = selectedObjects()[1]
		local class = obj and API.Classes[obj.ClassName]
		if not class then return nil end
		local tags = {}
		for tag in pairs(class.Tags or {}) do tags[#tags+1] = tag end
		table.sort(tags)
		return {Name = class.Name, Superclass = class.Superclass and class.Superclass.Name, Tags = tags,
			PropertyCount = #(class.Properties or {}), FunctionCount = #(class.Functions or {}), EventCount = #(class.Events or {})}
	end
	Main.Features.GetSelectionHealth = function()
		local result = {Count = 0, Missing = 0, NilParented = 0, Classes = {}}
		for _,node in ipairs(selectedNodes()) do
			result.Count = result.Count + 1
			if not node.Obj then result.Missing = result.Missing + 1 else
				if not node.Obj.Parent then result.NilParented = result.NilParented + 1 end
				result.Classes[node.Obj.ClassName] = (result.Classes[node.Obj.ClassName] or 0) + 1
			end
		end
		return result
	end
	Main.Features.StartReplayRecording = function()
		if not Replay or Replay.Recording then return false end
		Replay.StartRecording()
		return true
	end
	Main.Features.StopReplayRecording = function()
		if not Replay or not Replay.Recording then return false end
		Replay.StopRecording()
		return true
	end
	Main.Features.PlayReplay = function()
		if not Replay then return false end
		Replay.PlayReplay()
		return true
	end
	Main.Features.StopReplay = function()
		if not Replay then return false end
		Replay.StopPlayback()
		return true
	end
	Main.Features.OpenReplayStudio = function()
		if not Replay then return false end
		Replay.Window:Show()
		return true
	end
	Main.Features.ClearReplay = function()
		if not Replay then return false end
		Replay.StopRecording()
		Replay.StopPlayback()
		table.clear(Replay.Frames)
	...
	Main.Features.SaveReplay = function()
		if not Replay then return false end
		Replay.SaveReplay()
		return true
	end
	Main.Features.LoadReplay = function()
		if not Replay then return false end
		Replay.LoadReplay()
		return true
	end
	Main.Features.GetReplayStatus = function()
	...
	Main.SetupFilesystem = function()
		...
		for _,folder in ipairs({"dex","dex/assets","dex/saved","dex/plugins","dex/ModuleCache"}) do
			if not env.isfile or not env.isfile(folder) then pcall(makefolder,folder) end
		end
	end

	Main.SaveCurrentSettings = function()
		if env.writefile then
			local data = Main.ExportSettings()
			if data then pcall(env.writefile,"DexPlusPlusSettings.json",data) end
		end
	end
	...
	Main.Init = function()
		...
		Main.ApplyThemePreset(Settings.ThemeName or "Classic")
